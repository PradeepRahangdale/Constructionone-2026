...
export const getTrending = catchAsync(async (req, res) => {
    ...
    const cacheKey = trendingSectionService.trendingCacheKey(identifier, searchKeyword);
    const cached = await RedisCache.get(cacheKey);
    if (cached) {
        return res.status(200).json(new ApiResponse(200, cached, 'Trending fetched (cached)'));
    }

    // 2. Build trending layout/products 
    const data = await trendingSectionService.buildTrending(identifier, searchKeyword);
    ...
    return res.status(200).json(new ApiResponse(200, data, 'Trending fetched successfully'));
});
