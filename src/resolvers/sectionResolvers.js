...
const resolvePRODUCT_LIST = async (section) => {
    return Product.find(
        applySourceFilter({ moduleId: section.moduleId, disable: false }, section)
    )
        .sort({ createdAt: -1 })
        .limit(section.limit)
...
const resolveBRAND_LIST = async (section) => {
    return Brand.find(
        applySourceFilter({ moduleId: section.moduleId, status: 'active' }, section)
    )
        .sort({ order: 1 })
        .limit(section.limit)
...
const resolveFLASH_SALE = async (section) => {
    ...
    const enriched = items
        ...
        .map((item) => ({
            ...
            soldPercent: Math.max(0, Math.min(100, Math.round((item.sold / item.allocatedStock) * 100))),
        }));
